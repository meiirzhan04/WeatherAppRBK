        ...
        viewModelScope.launch {
            ...
            when (val locationResult = getCurrentLocationUseCase()) {
                is ResultState.Success -> {
                    val locationText = "lat = ${locationResult.data.latitude}, lon = ${locationResult.data.longitude}"
                    Log.d("LOCATION_RESULT", locationText)
                    setState { copy(locationText = locationText) }
                    when (val weatherResult = getCurrentWeatherUseCase(lat = locationResult.data.latitude, lon = locationResult.data.longitude)) {
                        is ResultState.Success -> {
                            Log.d("WEATHER_API", weatherResult.data.toString())
                ...
                is ResultState.Error -> { setState { copy(isLoading = false, error = locationResult.message) } }
                is ResultState.Loading -> { setState { copy(isLoading = true) } }
            }
        }
    }
